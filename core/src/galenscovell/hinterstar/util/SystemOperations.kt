...
import galenscovell.hinterstar.generation.sector.System
...
object SystemOperations {
    val systemsInRange: MutableList<System> = mutableListOf()
    val shapeRenderer: ShapeRenderer = ShapeRenderer()
    val playerRangeRadius: Double = 8.0 * Constants.SYSTEMMARKER_SIZE
    val systems: MutableList<System> = mutableListOf()

    lateinit var gameScreen: GameScreen
    var currentSystem: System? = null
    var currentSelection: System? = null
     ...
     */
    fun setup(game: GameScreen): Unit {
        gameScreen = game
    }
     ...
     * WORKAROUND IDEAS:
     */
    fun setTargetsInRange(): Unit {
        systemsInRange.clear()

        for (system in systems) {
            if (system != currentSystem) {
                val squareDist: Double =
                        Math.pow(currentSystem!!.getSystemMarker().sx.toDouble() - system.getSystemMarker().sx, 2.0) +
                        Math.pow(currentSystem!!.getSystemMarker().sy.toDouble() - system.getSystemMarker().sy, 2.0)
                if (squareDist <= Math.pow(playerRangeRadius, 2.0)) {
    ...
    fun drawShapes(): Unit {
        ...
        shapeRenderer.projectionMatrix = gameScreen!!.getInterfaceStage().camera.combined
        shapeRenderer.begin(ShapeRenderer.ShapeType.Line)

        shapeRenderer.setColor(0.95f, 0.61f, 0.07f, 0.6f)
        // shapeRenderer.circle(centerX, centerY, playerRangeRadius)
        shapeRenderer.circle(centerX, centerY, 30f)


        if (systemsInRange.isNotEmpty()) {
        ...
        if (currentSelection != null) {
            ...
            shapeRenderer.circle(selectionX, selectionY, 30f)
        }
     ...
     */
    fun parseNextEvent(): Event? {
        if (currentSystem != null) {
            return currentSystem!!.getEvent()
        } else {
            return null
        }
    }
     ...
     */
    fun setSelection(selection: System?): Unit {
        var distance: Int = 0

        if (selection != null) {
            currentSelection = selection
            distance = (4 * Math.ceil(
                    Math.sqrt(Math.pow(currentSystem!!.getSystemMarker().sx.toDouble() - selection!!.getSystemMarker().sx, 2.0) +
                            Math.pow(currentSystem!!.getSystemMarker().sy.toDouble() - selection!!.getSystemMarker().sy, 2.0))
            ) / Constants.SYSTEMMARKER_SIZE).toInt()
        }

        gameScreen.getInterfaceStage().updateDistanceLabel("Distance: $distance AU")
    }
     ...
     */
    fun populateSystems(systemsToSet: List<System>): Unit {
        systems.clear()

        for (s: System in systemsToSet) {
            systems.add(s)
        }
